...
final class ResourceMetaUtil {
  ...
  static Document generateDocumentFromResourceMetaDTO(final CreateResourceSchemaVO schemaVO) {
    ...
    for (Map.Entry<String, String> resourceMetaDisplayNameEntry :
        schemaVO.getResourceDisplayName().entrySet()) {
      displayNameDocument =
          displayNameDocument.append(
              resourceMetaDisplayNameEntry.getKey(), resourceMetaDisplayNameEntry.getValue());
    }
    document =
        document
            .append(MongoResourceConstant.RESOURCE_DISPLAY_NAME, displayNameDocument)
    ...
    schemaVO
        .getResourceFieldMetaList()
        .forEach(item -> resourceFieldDocList.add(generateDocumentFromResourceFieldMetaDTO(item)));

    document.put(MongoResourceConstant.RESOURCE_FIELD_LIST, resourceFieldDocList);

    // other logic
    //        document.put(MongoResourceConstant.RESOURCE_META_BYTE_STREAM, new
    // Binary(SerializationUtils.serialize(schemaVO)));
    //        document.put(MongoResourceConstant.RESOURCE_META_MAP_BYTE_STREAM, new
    // Binary(SerializationUtils.serialize(resourceMetaMap)));
    // todo need to change
  ...
  private static Document generateDocumentFromResourceFieldMetaDTO(
      ResourceFieldMetaDTO resourceFieldMetaDTO) {
    Document document =
        new Document()
    ...
    if (resourceFieldMetaDTO.getIndex() != null) {
      ...
      if (resourceFieldMetaDTO.getIndex().getName() != null) {
        indexDocument.put(
            ResourceFieldMetaIndex.INDEX_NAME, resourceFieldMetaDTO.getIndex().getName());
      }
      if (resourceFieldMetaDTO.getIndex().getUnique() != null) {
        indexDocument.put(
            ResourceFieldMetaIndex.INDEX_UNIQUE, resourceFieldMetaDTO.getIndex().getUnique());
      }
      if (resourceFieldMetaDTO.getIndex().getOrder() != null) {
        indexDocument.put(
            ResourceFieldMetaIndex.INDEX_ORDER, resourceFieldMetaDTO.getIndex().getOrder().name());
      }
      if (resourceFieldMetaDTO.getIndex().getText() != null) {
        indexDocument.put(
            ResourceFieldMetaIndex.INDEX_TEXT, resourceFieldMetaDTO.getIndex().getText());
      }
    ...
    if (resourceFieldMetaDTO.getDataType() == DataType.ARRAY) {
      document.put(
          MongoResourceConstant.NESTED_ARRAY_TYPE,
          resourceFieldMetaDTO.getNestedArrayType().name());
    }
  ...
  private static ResourceFieldMetaDTO getPredefinedFieldMetaForTags() {
    ...
    ResourceFieldMetaIndex resourceFieldMetaIndexDTO =
        new ResourceFieldMetaIndex(
            null, false, false, ResourceFieldMetaIndex.IndexOrder.DESCENDING);
  ...
  private static ResourceFieldMetaDTO getPredefinedFieldMetaForCreatedBy() {
    ...
    resourceFieldMetaDTO.setDataType(DataType.INTEGER);
    ResourceFieldMetaIndex resourceFieldMetaIndexDTO =
        new ResourceFieldMetaIndex(null, false, false, ResourceFieldMetaIndex.IndexOrder.ASCENDING);
    resourceFieldMetaDTO.setIndex(resourceFieldMetaIndexDTO);
    Map<String, String> descMap = new LinkedHashMap<>();
